...
class PowerScaleTests: XCTestCase {
    func testPowerScale_scale_identity() throws {
        let pow = ContinuousScale<CGFloat>(lower: 0, higher: 100, type: .power(1))
    ...
    func testPowerScale_scale_square() throws {
        let pow = ContinuousScale<CGFloat>(lower: 0, higher: 10, type: .power(2))
        guard let result = pow.scale(2, from: 0, to: 100) else {
            XCTFail()
            return
        }
    ...
    func testReversedRangeModifiers() {
        ...
        scale = ContinuousScale(lower: 0, higher: 100, reversed: true, rangeLower: 0, rangeHigher: 100)
    ...
    func testReversedCalculations() {
        let scale = ContinuousScale<CGFloat>(lower: 0, higher: 100, type: .power(1), reversed: true, rangeLower: 0, rangeHigher: 100)
    ...
    func testReversedTicks() {
        let reversed = ContinuousScale<CGFloat>(lower: 0, higher: 100, type: .power(1), reversed: true, rangeLower: 0, rangeHigher: 100)
