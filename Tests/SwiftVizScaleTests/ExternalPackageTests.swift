...
final class PackagingTests: XCTestCase {
    func testManualTicks() {
        let scale = ContinuousScale<Double, CGFloat>(from: 0.0, to: 10.0, transform: .none)
    ...
    func testManualTicksOutsideRangeClamped() {
        let scale = ContinuousScale<Double, CGFloat>(from: 0.0, to: 10.0, transform: .clamp)
        let ticks = scale.ticksFromValues([2.0, 4.0, 8.0, 16.0], from: 0.0, to: 10.0)
        XCTAssertEqual(ticks.count, 3)
    }

    func testManualTicksOutsideRangeDropped() {
        let scale = ContinuousScale<Double, CGFloat>(from: 0.0, to: 10.0, transform: .drop)
        let ticks = scale.ticksFromValues([2.0, 4.0, 8.0, 16.0], from: 0.0, to: 10.0)
        XCTAssertEqual(ticks.count, 3)
    }
    ...
    func testScaleTransform() {
        ...
        let cScale = ContinuousScale<Double, CGFloat>(from: 5.0, to: 10.0, transform: .clamp)
        XCTAssertEqual(inputs.map { cScale.transformAgainstDomain($0) }, [10.0, 5.0, 7.0])

        let dScale = ContinuousScale<Double, CGFloat>(from: 5.0, to: 10.0, transform: .drop)
